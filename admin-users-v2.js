...
function badge(u){
 const st=statusOf(u),on=online(u),del=String(u.deletionStatus||"").toLowerCase()==="pending";
 const delText={ht:"Sipresyon mande",fr:"Suppression demandée",en:"Deletion requested",es:"Eliminación solicitada"}[lang()]||"Suppression demandée";
 let label=on?tr("online"):tr("offline");
 if(del)label=delText;else if(st==="blocked")label=tr("blocked");else if(st==="deleted")label=tr("deleted");else if(st==="rejected")label=tr("rejected");
 return`<span class="presence ${del?"deletion-pending":(on?"online":"offline")}">${esc(label)}</span><small class="presence-age">${esc(tr("signal"))}: ${esc(ageText(u))}</small>`;
}
