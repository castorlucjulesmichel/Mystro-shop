...
import{getFirestore,doc,getDoc,collection,getDocs,getDocsFromServer,query,limit}from"https://www.gstatic.com/firebasejs/10.14.1/firebase-firestore.js";
...
function renderSnapshot(s){if(!s)return;const set=(id,v)=>{const el=$(id);if(el&&v!==undefined&&v!==null)el.textContent=v};set("adminUsersCount",s.userCount);set("adminSellersCount",s.sellerCount);set("adminProductsCount",s.productCount);set("adminOrdersCount",s.orderCount);set("adminPaidOrdersCount",s.paidCount);if(s.commission!==undefined)set("adminCommissionTotal",moneyHTG(s.commission));renderList($("adminUsersList"),s.recentUsers||[],"user");renderList($("adminOrdersList"),s.recentOrders||[],"order")}
function timeout(p,ms=10000){return Promise.race([p,new Promise((_,rej)=>setTimeout(()=>rej(Error("TIMEOUT")),ms))])}
async function fresh(ref){try{return await timeout(getDocsFromServer(query(ref,limit(MAX_DOCS))),10000)}catch(e){console.warn("server read fallback",e);return timeout(getDocs(query(ref,limit(MAX_DOCS))),7000)}}
const docs=s=>s.docs.map(d=>({id:d.id,...d.data()}));
const newest=(arr,n=8)=>[...arr].sort((a,b)=>ts(b.createdAt)-ts(a.createdAt)).slice(0,n);

async function loadAdminData(force=false){
 if(loading||!auth.currentUser)return;
 const cached=readCache();if(cached&&!force)renderSnapshot(cached);
 loading=true;if(force&&status)status.textContent=tr("loading");
 try{
  const [us,ps,os]=await Promise.all([fresh(collection(db,"users")),fresh(collection(db,"products")),fresh(collection(db,"orders"))]);
  const users=docs(us),products=docs(ps),orders=docs(os);
  const paid=orders.filter(o=>["paid","completed","success"].includes(String(o.status||"").toLowerCase())||String(o.paymentStatus||"").toLowerCase()==="settled");
  let commission=0;for(const o of paid){const direct=Number(o.platformCommission??o.platformFee??o.commission);commission+=Number.isFinite(direct)&&direct>0?direct:Number(o.subtotal??o.totalProducts??o.total??0)*.10}
  const next={savedAt:Date.now(),userCount:users.length,sellerCount:users.filter(u=>String(u.role||"").toLowerCase()==="seller").length,productCount:products.length,orderCount:orders.length,paidCount:paid.length,commission,recentUsers:newest(users),recentOrders:newest(orders)};
  writeCache(next);renderSnapshot(next);if(status)status.textContent="";
  document.dispatchEvent(new CustomEvent("adminOverviewLoaded",{detail:next}));
 }catch(e){console.error(e);if(cached)renderSnapshot(cached);if(status)status.textContent=tr("loadError")}finally{loading=false}
}
...
async function verifyAdmin(user){if(!user){showLogin();return}try{const snap=await timeout(getDoc(doc(db,"users",user.uid)),8000),role=String(snap.data()?.role||"").toLowerCase();if(role!=="admin"){await signOut(auth);showLogin(tr("denied"));return}showDashboard(user);await loadAdminData(true);startAutoRefresh()}catch(e){console.error(e);showLogin(tr("loadError"))}}
