...
function switchAuthMode(mode) {
  ...
  if (loginForm) {
    ...
    loginForm.style.display =
      showLogin ? "block" : "none";
  }

  if (signupForm) {
    signupForm.hidden = showLogin;
    signupForm.style.display =
      showLogin ? "none" : "block";
  }
...
function showPage(pageId) {
  ...
  /*
    Portefeuille est disponible
    aussi pour les acheteurs.
  */
...
async function uploadProductImage(file) {
  ...
  const fileName =
    `${currentUser.uid}/${Date.now()}-${crypto.randomUUID()}.${extension}`;
