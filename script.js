...
const translations = {
  ...
  fr: {
    ...
    welcome: "Mystro-Shop — Marché international",
    welcomeText: "Achetez et vendez partout dans le monde.",
  ...
  es: {
    ...
    sellerBalance:
      "Saldo después de la comisión del 10%",
...
function assistantAnswer(message) {
  ...
  if (
    /depot|dépôt|deposit|depoz/.test(text)
  ) {

    return language === "ht"
      ? "Ale nan Pòtfèy epi peze Depoze pou chwazi montan MonCash la."
      : "Ouvrez Portefeuille puis Déposer pour choisir le montant MonCash.";
  }


  if (
    /retrait|withdraw|retire/.test(text)
  ) {

    return language === "ht"
      ? "Ale nan Pòtfèy, peze Retire, mete montan ak nimewo MonCash la."
      : "Ouvrez Portefeuille, appuyez sur Retirer, puis entrez le montant et le numéro MonCash.";
  }


  if (
    /vend|vann|sell/.test(text)
  ) {

    return language === "ht"
      ? "Ale nan Vann, mete enfòmasyon pwodwi a ak foto li epi pibliye li."
      : "Ouvrez Vendre, ajoutez les informations et la photo du produit puis publiez.";
  }


  if (
    /commission|10%/.test(text)
  ) {
